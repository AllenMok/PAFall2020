...
def plot_df_2var(output,x1, y1, x2, y2, title='', xlabel='Time', y1label='Cases',y2label='Cases',line1 = 'line1',line2 = 'line2', dpi=100,diffy = True):
    #plot 2 var in one plot
...
def replotTable(out,df,lag = 31):
    #replot var table to comparing with covid cases, with time lag
    df = df[40:]
    case = df.cases[lag:]
...
def replotCrash(out,df,lag = 31):
    #replot var traffic crashes to comparing with covid cases, with time lag
    df = df[40:]
    case = df.cases[lag:]
    case.index = range(len(case))
    crash = df.crashes
...
def replotTrend(out,df,lag = 31):
    #replot var google trends to comparing with covid cases, with time lag
    df = df[40:]
    case = df.cases[lag:]
    case.index = range(len(case))
    trends = df.trends
